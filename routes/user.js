...
router.get('/assessment_test.x',verifyUser, function (req, res, next) {

    res.render('user/test-1to4');
  
});

router.get('/assessment_test.y',verifyUser, function (req, res, next) {
  res.render('user/test-5to8');
});

router.get('/assessment_test.z',verifyUser, function (req, res, next) {
...
date_format.second = 'numeric';
 ...
 if (q10===10){
  ...
  if (user.class === '1to4') {
    ...
    user.uniqueid = 'RH'+user.indexno+user.level+user.scholar
